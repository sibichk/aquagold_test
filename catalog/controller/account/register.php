...
class ControllerAccountRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($this->request->post['code']) {
				$voucher = $this->model_checkout_voucher->getVoucher($this->request->post['code']);
				$credit = $voucher['amount'] - $voucher['amount']*5/100;
				$this->model_checkout_voucher->transaction($voucher['order_id'], 'Promo code', $credit);
				$this->model_checkout_voucher->reward($this->request->post['code'], $voucher['order_id'], 'Promo code', $voucher['amount']/10);
				$this->session->data['promo_message'] = '<b>Your coupon has been redeemed!</b>';
			}
	...
	protected function validate() {
		...
		if ($this->request->post['code'] && !$this->model_checkout_voucher->getVoucher($this->request->post['code'])) {
