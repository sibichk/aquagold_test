...
class ControllerAccountTransaction extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/transaction.tpl')) {
			if($this->data['type'] == 1)
			$this->template = $this->config->get('config_template') . '/template/account/transaction_physician.tpl';
			else if($this->data['type'] == 2)
			$this->template = $this->config->get('config_template') . '/template/account/transaction_patient.tpl';
			else
