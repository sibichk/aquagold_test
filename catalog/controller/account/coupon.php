...
class ControllerAccountCoupon extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//$this->redirect($this->url->link('account/coupon', $url, 'SSL'));
			$this->redirect($this->url->link('account/voucher', $url, 'SSL'));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $coupon_id) {
				$this->model_account_coupon->deleteCoupon($coupon_id);
			}
	...
	protected function getList() {
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('account/coupon', $url, 'SSL'),
			'separator' => $this->language->get('text_separator')
		);

		$this->data['insert'] = $this->url->link('account/coupon/insert', $url, 'SSL');
		$this->data['delete'] = $this->url->link('account/coupon/delete', $url, 'SSL');
		...
		$results = $this->model_account_coupon->getCoupons($data);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('account/coupon/update', 'coupon_id=' . $result['coupon_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = HTTPS_SERVER . 'index.php?route=account/coupon&sort=name' . $url;
		$this->data['sort_code'] = HTTPS_SERVER . 'index.php?route=account/coupon&sort=code' . $url;
		$this->data['sort_discount'] = HTTPS_SERVER . 'index.php?route=account/coupon&sort=discount' . $url;
		$this->data['sort_date_start'] = HTTPS_SERVER . 'index.php?route=account/coupon&sort=date_start' . $url;
		$this->data['sort_date_end'] = HTTPS_SERVER . 'index.php?route=account/coupon&sort=date_end' . $url;
		$this->data['sort_status'] = HTTPS_SERVER . 'index.php?route=account/coupon&sort=status' . $url;
		...
		$pagination->url = HTTPS_SERVER . 'index.php?route=account/coupon' . $url . '&page={page}';
		...
		$this->template = 'aquagold/template/account/coupon_list.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['coupon_id'])) {
			$this->data['action'] = $this->url->link('account/coupon/insert', $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('account/coupon/update', 'coupon_id=' . $this->request->get['coupon_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('account/coupon', $url, 'SSL');
		...
		$this->template = 'aquagold/template/account/coupon_form.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
		);
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10; 
		$pagination->url = $this->url->link('account/coupon/history', 'coupon_id=' . $this->request->get['coupon_id'] . '&page={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();

		$this->template = 'aquagold/template/account/coupon_history.tpl';		
