...
class ControllerAccountOrder extends Controller {
	...
	public function index() {
		...
		if($this->data['type'] == 2) {
			$results = $this->model_account_order->getVouchers(($page - 1) * 10, 10);
			$order_total = $this->model_account_order->getTotalVouchers();
		} else {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/order_list.tpl')) {
			if($this->data['type'] == 1)
			$this->template = $this->config->get('config_template') . '/template/account/order_list_physician.tpl';
			else if($this->data['type'] == 2)
			$this->template = $this->config->get('config_template') . '/template/account/order_list_patient.tpl';
			else
