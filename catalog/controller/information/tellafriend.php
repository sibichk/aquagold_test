...
class ControllerInformationTellafriend extends Controller {
	...
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('information/tellafriend', '', 'SSL');
    ...
    if ((strlen(utf8_decode($this->request->post['name'])) < 3) || (strlen(utf8_decode($this->request->post['name'])) > 32)) {
      $this->error['name'] = $this->language->get('error_name');
		...
		if ((strlen(utf8_decode($this->request->post['email'])) < 10) || (strlen(utf8_decode($this->request->post['email'])) > 3000)) {
