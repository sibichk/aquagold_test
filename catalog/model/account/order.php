...
class ModelAccountOrder extends Model {
	...
	public function getVouchers($start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, os.name as status, o.date_added, o.total, o.currency_code, o.currency_value FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) JOIN `" . DB_PREFIX . "order_voucher` ov ON o.order_id = ov.order_id WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND o.order_status_id > '0' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);	

		return $query->rows;
	}
	
	public function getTotalVouchers() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` a JOIN `" . DB_PREFIX . "order_voucher` b ON a.order_id = b.order_id WHERE a.customer_id = '" . (int)$this->customer->getId() . "' AND a.order_status_id > '0'");
